...
# ===================== TAYYAB: START — cost guard =====================
# Problem this solves: extract_memories() calls the LLM on EVERY message,
# even "hi" or "ok". That's a second full API call per chat turn, doubling
# cost for no reason. This filter runs BEFORE that LLM call and skips it
# for messages that couldn't plausibly contain a fact worth saving.

_TRIVIAL_REPLIES = {
    "yes", "no", "ok", "okay", "k", "kk", "sure", "yeah", "yep", "nope",
    "thanks", "thank you", "thx", "cool", "nice", "great", "hi", "hey",
    "hello", "hii", "bye", "goodbye", "lol", "haha",
}


def _looks_worth_remembering(message: str) -> bool:
    """
    Cheap pre-filter so we don't burn a full LLM call on every single
    message just to check whether it contains a saveable fact.

    Only messages that pass this go on to extract_memories() (which
    calls the LLM). "yes, he is my uncle" still passes — it's longer
    than 6 chars and isn't an exact trivial-reply match.
    """
...
def _persist_memory(
    ...
    """
    # ===== TAYYAB: added this guard — skips the LLM call for trivial messages =====
    if not _looks_worth_remembering(message):
        return
    # ===== TAYYAB: end =====
